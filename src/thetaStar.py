...
def thetaStar(start, goal, grid, heur='naive'):
    ...
    while openset:
        ...
        for node in children(current,grid):
            ...
            if node in openset:
                if LineOfSight(current.parent, node, grid) and current.parent is not None:
                    '''Path 2'''
                    if (current.parent.G + current.parent.move_cost(node)) < node.G:
                        node.G = current.parent.G + current.parent.move_cost(node)
                        node.parent = current.parent
                        if node not in openset: #Posible cambio not in
                            #node.G = current.G + current.move_cost(node) Igual no es con remove()
                            openset.remove(node)
                        openset.add(node)
                else:
                    '''Path 1'''
